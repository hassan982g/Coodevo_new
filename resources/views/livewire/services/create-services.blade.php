            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <form method="POST" wire:submit="save">
                    ...
                    <div class="mt-4 hidden">
                        <label for="lni_icon" class="block text-sm font-medium text-gray-700">Lni icon</label>
                        <input id="lni_icon" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm"
                            type="text" wire:model="form.lni_icon" />
                        @error('form.lni_icon')
                            <span class="mt-2 text-sm text-red-600">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mt-4">
                        <label for="photo" class="block text-sm font-medium text-gray-700">Photo (800 * 500 recomended)</label>
                        <x-dropzone id="photo" name="photo" action="{{ route('model.storeMedia') }}" collection-name="service_photo" model-name="service" max-file-size="2" max-width="4096" max-height="4096" max-files="1" />
                        @error('form.mediaCollections.service_photo')
                            <span class="mt-2 text-sm text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="keywords" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm"
                            type="text" wire:model="form.keywords" />
                        @error('form.keywords')
                            <span class="mt-2 text-sm text-red-600">{{ $message }}</span>
                        @enderror
