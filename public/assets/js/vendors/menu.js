...
menuMain.addEventListener("click", (e) => {
  ...
  if (e.target.closest(".nav-item-has-children")) {
    var hasChildren = e.target.closest(".nav-item-has-children");

    showSubMenu(hasChildren);
  }
});
goBack.addEventListener("click", () => {
  var lastItem = last(subMenuArray);
  var lastItemText = last2(subMenuTextArray);
  subMenuArray.pop();
...
function showSubMenu(hasChildren) {
  for (var i = 0; submenuAll.length < i; i++) {
  ...
  var menuTitle = hasChildren.querySelector(".drop-trigger").textContent;
